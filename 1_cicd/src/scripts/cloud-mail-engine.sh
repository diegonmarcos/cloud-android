...
step_materialize_fork() {
  # THIS IS OUR CODE. aa_cloud-mail/ IS the gradle project -- app/, build.gradle,
  ...
  if [ ! -f "$dir/build.gradle" ] || [ ! -d "$dir/app" ]; then
    ...
    errlog "  aa_cloud-mail IS the source tree -- nothing clones it into place."
    exit 1
  fi
...
step_build_fork() {
  ...
  # our own in-repo tree (aa_cloud-mail IS the gradle project; since
