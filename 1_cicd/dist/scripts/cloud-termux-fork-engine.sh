...
_step_build_single_app() {
  ...
  [ "$DIST_DIR/cloud-terminal.apk" = "$out" ] || cp "$DIST_DIR/cloud-terminal.apk" "$out"
  log "→ $out"
}
...
step_build_fork() {
  ...
  if { [ -z "$task" ] || [ "$task" = "null" ]; } && { [ -z "$bcmd_gate" ] || [ "$bcmd_gate" = "null" ]; }; then
    ...
    mkdir -p "$DIST_DIR"
    _fetch_upstream_apk "$key" "$up_url" "$up_sha" "$up_resign" "$DIST_DIR/cloud-terminal.apk" \
      || { errlog "build-fork[$key]: upstream APK fetch/resign failed"; exit 1; }
    _enforce_signature "$DIST_DIR/cloud-terminal.apk"
    ...
    _assert_apk_identity "$key" "$DIST_DIR/cloud-terminal.apk" "$up_pkg"
    log "build-fork[$key]: upstream-APK fork ($bundle_abi) → $DIST_DIR/cloud-terminal.apk ($(wc -c <"$DIST_DIR/cloud-terminal.apk") B)"
    return 0
  fi
  ...
  cp "${apks[0]}" "$DIST_DIR/cloud-terminal.apk"
  ...
  if [ "$(_fork_json "$key" ".build.resign_unsigned")" = "true" ]; then
    ...
    _resign_apk "$DIST_DIR/cloud-terminal.apk" "$DIST_DIR/cloud-terminal.apk.signed" \
      && mv "$DIST_DIR/cloud-terminal.apk.signed" "$DIST_DIR/cloud-terminal.apk" \
      || { errlog "build-fork[$key]: resign failed"; exit 1; }
  fi
  _enforce_signature "$DIST_DIR/cloud-terminal.apk"
  _assert_apk_identity "$key" "$DIST_DIR/cloud-terminal.apk"
  log "→ $DIST_DIR/cloud-terminal.apk ($(wc -c <"$DIST_DIR/cloud-terminal.apk") B)"
}
...
step_publish_fork() {
  ...
  artifact="$DIST_DIR/cloud-terminal.apk"
  ...
  for tag in latest "sha-${sha:0:8}"; do
    ...
    ( cd "$DIST_DIR" && in_nix oras push "$ref" "cloud-terminal.apk:$media_type" \
...
step_gh_release_fork() {
  ...
  src="$DIST_DIR/cloud-terminal.apk"
  ...
  log "gh-release-fork[$key]: upload cloud-terminal.apk → $rolling_tag"
