...
public class AdapterNavAccountFolder extends RecyclerView.Adapter<AdapterNavAccountFolder.ViewHolder> {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnLongClickListener {
        ...
        private void bindTo(TupleAccountFolder account) {
            ...
            // comms: the last sync time moved to tvSync at the START of the row
            // (see item_nav.xml). It is an account-level fact, so it reads as a
            // prefix to the account name rather than as one more of the row's
            // right-hand counters, which is where it used to get lost.
            if (account.folderName == null && account.last_connected != null && expanded && nav_last_sync) {
                ...
                if (account.last_connected < cal.getTimeInMillis())
                    tvSync.setText(DF.format(account.last_connected));
                else
                    tvSync.setText(TF.format(account.last_connected));
                tvSync.setTextColor(textColorSecondary);
