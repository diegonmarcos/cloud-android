...
**`ac_cloud-mail/build.sh` is a symlink** to
`~/git/cloud-u-android/1_cicd/src/scripts/cloud-media-center-fork-engine.sh`. That engine
...
**`ac_cloud-ide/build.sh`** is a standalone ~22K non-symlinked engine, but it
...
2. Follow the `ac_cloud-ide` standalone pattern but strip the hub/bundle
...
tracker checkout (`../ac_upstreams-sources/media-refra` at tag
